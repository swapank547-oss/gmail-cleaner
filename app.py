    ...
    def _run(tid):
        try:
            ...
            for cat in categories:
                ...
                try:
                    cat_service = _get_service()
                    if not cat_service:
                        results[cat] = {"total": 0, "senders": {}, "error": "Session expired"}
                        continue

                    resp = cat_service.users().messages().list(
                    ...
                    sampled = sample_ids[:300]
                    if sampled:
                        for i, mid in enumerate(sampled):
                            if i % 50 == 0 and i > 0:
                                _task_save(tid, {
                                    "status": "scanning",
                                    "categories": results,
                                    "overall_total": overall,
                                    "current": f"Fetching senders in {cat}... ({i}/{len(sampled)})",
                                })
                            try:
