...
def oauth2callback():
    ...
    try:
        ...
        _save_credentials(creds)
        app.logger.info("OAUTH SUCCESS: creds saved, valid=%s, expiry=%s", creds.valid, creds.expiry)
    except Exception as e:
        app.logger.error("OAUTH FAILED: %s", str(e))
    return redirect(url_for("index"))


@app.route("/sess")
def sess():
    raw = session.get("gmail_creds")
    parsed = None
    if raw:
        try:
            creds = Credentials.from_authorized_user_info(json.loads(raw), SCOPES)
            parsed = {"valid": creds.valid, "expired": creds.expired, "has_refresh": bool(creds.refresh_token)}
        except Exception as e:
            parsed = {"error": str(e)}
    return jsonify({
